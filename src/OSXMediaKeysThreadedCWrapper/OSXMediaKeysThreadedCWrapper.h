/**
 * mktcw = Media Keys Threaded C Wrapper
 *
 ...
 * API is the "osx-media-keys" module located under /lib/osx-media-keys.js which
 * accesses this API through two ChromeWorker threads
 * (/data/OSXMediaKeysControllerWorker.js and -ListenerWorker.js).
 *
 * The API and implementation of this wrapper is almost exactly the same as
 * AppleRemoteThreadedCWrapper. If you've understood one of them you've
 ...
 * Since the Media Keys library which we're wrapping around can only be used
 ...
 * This is what mktcw_get_message does; it blocks until an event arrives and
 * returns it as soon as that happens.
 * Note that this means that we need a second worker thread since we also want
 * to be able to call startListening/stopListening on the OSXMediaKeys while
 * we're blocking in mktcw_get_message. That's why there are two JavaScript
 * ChromeWorkers: ControllerThread and ListenerThread.
 * We want to terminate ListenerThread when the OSXMediaKeys module is shut
 * down. For that we have an "isDestroyNotification" flag on the events that
 * are returned by mktcw_get_message so we don't have to terminate the thread
...
void* mktcw_create();
void mktcw_start_listening(void* arctw);
void mktcw_stop_listening(void* arctw);
void mktcw_destroy(void* arctw);

typedef struct mktcw_message
{
...
mktcw_message mktcw_get_message(void* arctw);
